    ...
    private static SpeechTranscriberListener getTranscriberListener() {
        SpeechTranscriberListener listener = new SpeechTranscriberListener() {
            ...
            public void onTranscriptionResultChange(SpeechTranscriberResponse response) {
                ...
                log.info("onTranscriptionResultChange"+response.toString());
...
//                System.out.println("task_id: " + response.getTaskId() + ", name: " + response.getName() + ", status: " + response.getStatus());
                log.info("onTranscriberStart {}",response.toString());
            }
...
//                System.out.println("task_id: " + response.getTaskId() + ", name: " + response.getName() + ", status: " + response.getStatus());
                ...
                log.info("onSentenceBegin {}",response.toString());
            }

            //识别出一句话.服务端会智能断句,当识别到一句话结束时会返回此消息
            @Override
            public void onSentenceEnd(SpeechTranscriberResponse response) {
                log.info("onSentenceEnd"+response.toString());
//                System.out.println("task_id: " + response.getTaskId() +
//                    ", name: " + response.getName() +
//                    //状态码 20000000 表示正常识别
//                    ", status: " + response.getStatus() +
...
//                System.out.println("task_id: " + response.getTaskId() + ", name: " + response.getName() + ", status: " + response.getStatus());
                log.info("onTranscriptionComplete {}",response.toString());
            }
...
//            logger.info("ASR latency : " + (System.currentTimeMillis() - now) + " ms");
            ...
            log.info("speech error "+e.getMessage());
        } finally {
            if (null != transcriber) {
                transcriber.close();
            }
...
//        String filepath = "nls-sample-16k.wav";
        String filepath = "D:\\a\\0126\\a4.wav";
